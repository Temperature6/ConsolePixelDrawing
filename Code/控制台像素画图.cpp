...
2021/02/19 13:02-去掉了/f参数，可拖动画布文件到.exe上直接打开
2021/03/28 11:22-优化了逻辑，保存时按打开画布的路径保存
*/
#include <iostream>
#include <conio.h>
#include <string>
...
START:
	...
	{ //打开画布文件
		...
		{
			ReadProject(temparr, argv[1]);
			memcpy(path, argv[1], sizeof(path));
			getname(argv[1], name);
		}
